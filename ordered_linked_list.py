class OrderedLinkedList :
    ...
    def __init__(self, lst=[]):
        ...
        lst.sort()              # reverse to ensure elements will appear in same order
    ...
    def remove_middle(self, cargo):
        """
        Removes a node with the given cargo from the list.
        If the node does not exist, the list remains unchanged.
        """
        if self.size() == 0:
            return
        ...
        previous = None
        current = self.first()
        while current is not None and current.value() != cargo:
            previous = current
    ...
    def add_middle(self, cargo):
        """
        Adds a node in the middle of the list while maintaining it ordered.
        """
        node = self.Node(cargo)

        if self.size() == 0 or node.value() <= self.first().value():
            self.add(cargo)
            self.inc_size()
            return
        ...
        while current.next() is not None and node.value() > current.next().value():
            current = current.next()

        current.add_after(node)
        self.inc_size()

    def to_list(self):
        """
        It returns a list with the Node values in the Linked list
